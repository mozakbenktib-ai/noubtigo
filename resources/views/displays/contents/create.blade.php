...
@section('title', __('ui.create_display_content') . ' | Noubtigo')
@section('header_title', __('ui.create_display_content'))
@section('header_subtitle', __('ui.create_display_content_subtitle'))
...
<div class="container-fluid px-0" style="max-width: 980px;">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <a href="{{ route('displays.contents.index') }}" class="btn btn-light rounded-pill px-3 fw-semibold text-secondary">
            <i class="bi bi-arrow-left me-1"></i> {{ __('ui.back_to_display_contents') }}
        </a>
    </div>

    <form action="{{ route('displays.contents.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div class="card-body p-4">
                        ...
                        @foreach($types as $key => $t)
                            <div class="col-md-3 col-6">
                                <label class="w-100 h-100 cursor-pointer">
                                    <input type="radio" name="type" value="{{ $key }}" class="btn-check" id="type_{{ $key }}" {{ old('type', 'information') === $key ? 'checked' : '' }} onchange="onTypeChanged('{{ $key }}')">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" class="form-control rounded-3 @error('description') is-invalid @enderror" rows="3" 
                              placeholder="{{ __('ui.description_placeholder') }}">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3" id="image-upload-section">
                    <label class="form-label small fw-bold text-uppercase text-secondary">{{ __('ui.visual_image_banner') }}</label>
                ...
                <div class="mb-3" id="link-section">
                    ...
                    <div class="input-group">
                        ...
                        <input type="url" name="link_url" class="form-control rounded-end-3" placeholder="https://example.com/promotion" value="{{ old('link_url') }}">
                    </div>
                    <div class="form-text small">{{ __('ui.optional_link_hint') }}</div>
                </div>

                <!-- QR / Tracking Specific URL -->
                <div class="mb-3" id="qr-section">
                    <label class="form-label small fw-bold text-uppercase text-secondary">{{ __('ui.custom_qr_url') }}</label>
                    <div class="input-group">
                        <span class="input-group-text bg-light border-end-0 rounded-start-3"><i class="bi bi-qr-code"></i></span>
                        <input type="url" name="qr_url" class="form-control rounded-end-3" placeholder="https://..." value="{{ old('qr_url') }}">
                    </div>
                    <div class="form-text small text-primary">
                        <i class="bi bi-info-circle me-1"></i> {{ __('ui.custom_qr_hint') }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4">
            ...
            <div class="card-body p-4">
                    ...
                    <div class="d-flex gap-4">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="target_type" id="target_all" value="all" 
                                   {{ old('target_type', 'all') === 'all' ? 'checked' : '' }} onchange="toggleTargetDisplays(false)">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="target_type" id="target_selected" value="selected" 
                                   {{ old('target_type') === 'selected' ? 'checked' : '' }} onchange="toggleTargetDisplays(true)">
                ...
                <div id="selected-displays-box" class="border rounded-4 p-3 bg-light bg-opacity-50 mt-3 {{ old('target_type') === 'selected' ? '' : 'd-none' }}">
            ...
            <div class="card-body p-4">
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <input type="number" name="duration" class="form-control rounded-start-3" min="3" max="300" 
                                   value="{{ old('duration', 10) }}" id="durationInput" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="sort_order" class="form-control rounded-3" min="0" value="{{ old('sort_order', 0) }}">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="priority" class="form-select rounded-3">
                            <option value="normal" {{ old('priority', 'normal') === 'normal' ? 'selected' : '' }}>{{ __('ui.priority_normal') }}</option>
                            <option value="high" {{ old('priority') === 'high' ? 'selected' : '' }}>{{ __('ui.priority_high') }}</option>
                            <option value="emergency" {{ old('priority') === 'emergency' ? 'selected' : '' }}>{{ __('ui.priority_emergency') }}</option>
                        </select>
                    ...
                    <div class="col-md-6 mt-3">
                        ...
                        <input type="datetime-local" name="ends_at" class="form-control rounded-3" value="{{ old('ends_at') }}">
                        ...
                        <div class="form-check form-switch p-0">
                            <label class="form-check-label fw-bold text-dark d-flex align-items-center gap-3 cursor-pointer" for="is_active">
                                <input class="form-check-input ms-0 float-none" type="checkbox" name="is_active" value="1" id="is_active" 
                                       {{ old('is_active', '1') ? 'checked' : '' }} style="width: 2.5em; height: 1.3em;">
                                <span>{{ __('ui.publish_immediately') }}</span>
                            </label>
                        </div>
                    </div>
                </div>
        ...
        <div class="d-flex align-items-center justify-content-end gap-3 mb-5">
            ...
            <button type="submit" class="btn btn-primary rounded-pill px-5 py-2 fw-bold shadow-sm">
                <i class="bi bi-check-circle-fill me-1"></i> {{ __('ui.save_display_content') }}
            </button>
        </div>
    </form>
</div>
